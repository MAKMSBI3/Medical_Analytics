{{ config(materialized='table', schema='gold', tags=['gold','fact'], description='Billing fact table with financial context') }}
...
FROM {{ ref('silver_billing') }} b
LEFT JOIN {{ ref('fact_appointments') }} a ON b.appointment_id = a.appointment_id
LEFT JOIN {{ ref('dim_patients') }} p ON b.patient_id = p.patient_id
